...
/***/ (function(module, exports, __webpack_require__) {
	...
	/**
	 * Created by LiRong on 2017/11/18.
	 */
...
/***/ (function(module, exports, __webpack_require__) {
	...
	__webpack_require__(7) && $export($export.P + __webpack_require__(272), 'Object', {
	...
	var Lottery = function (_mix) {
	    ...
	    _createClass(Lottery, [{
	        ...
	        value: function initEvent() {
	            ...
	            //号码
	            (0, _jquery2.default)('#confirm_sel_code').on('click', self.addCode.bind(self)); //确认选号，清空号码
	            (0, _jquery2.default)('.dxjo').on('click', 'li', self.assistHandle.bind(self));
	            //全，大小奇偶
	            (0, _jquery2.default)('.qkmethod').on('click', '.btn-middle', self.getRandomCode.bind(self)); //机选
	        }
	    }]);
	...
	var Base = function () {
	    ...
	    _createClass(Base, [{
	        ...
	        value: function toggleCodeActive(e) {
	            var self = this;
	            var $cur = (0, _jquery2.default)(e.currentTarget); //取出当前选出的dom
	        ...
	        value: function addCode() {
	            ...
	            var count = self.computeCount(active, self.cur_play); //注数
	            if (count) {
	        ...
	        value: function addCodeItem(code, type, typeName, count) {
	            ...
	            (0, _jquery2.default)(self.cart_el).append(tpl); //向每个匹配的元素内部追加内容。
	            self.getTotal();
	        }
	    }, {
	        key: 'getCount',
	        value: function getCount() {
	            ...
	            var count = self.computeCount(active, self.cur_play); //柱数
	            var range = self.computeBonus(active, self.cur_play); //奖金[min,max]
	            var money = count * 2;
	            var win1 = range[0] - money; //最小盈利
	            var win2 = range[1] - money; //最大盈利
	            var tpl = void 0;
	            var c1 = win1 < 0 && win2 < 0 ? Math.abs(win1) : win1; //返回绝对值，盈利情况
	        ...
	        value: function getRandomCode(e) {
	            e.preventDefault();
	            var num = e.currentTarget.getAttribute('count'); //当前生成的数量
	            var play = this.cur_play.match(/\d+/g)[0]; //当前玩法的数字。
